...
function CartItem({ id, media, name, quantity, line_total, selected_options }) {
  ...
  return (
    <div className="py-3 md:py-4 lg:py-5 flex items-end space-x-3 md:space-x-4 lg:space-x-5 border-b border-black">
      <div className="w-12 h-12 lg:w-14 lg:h-14 rounded relative">
        <Image
          src={media.source}
          alt={name}
          layout="fill"
      ...
      <div className="flex flex-row items-end flex-grow">
        ...
        <div className="flex flex-col items-start items-end justify-between flex-grow">
          <div className="text-lg md:text-xl lg:text-2xl">
            {line_total.formatted_with_symbol}
          </div>
          <div className="w-full flex flex-col items-end justify-between">
            <div className="inline-flex items-center">
              <span className="pr-2">Quantity:</span> {quantity}
              { 
              /*
